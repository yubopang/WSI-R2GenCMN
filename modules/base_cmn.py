...
class BaseCMN(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn, max_len):
        c = copy.deepcopy
        attn = MultiHeadedAttention(self.num_heads, self.d_model)
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        position = PositionalEncoding(self.d_model, self.dropout, max_len=max_len)
        model = Transformer(
    ...
    def __init__(self, args, tokenizer):
        ...
        max_len = max(getattr(args, 'max_fea_length', 0), getattr(args, 'max_seq_length', 0), 5000)
        self.model = self.make_model(tgt_vocab, self.cmn, max_len=max_len)
