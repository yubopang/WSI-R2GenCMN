...
from models.r2gencmn import R2GenCMNModel
import warnings
...
def parse_agrs():
    ...
    parser.add_argument('--dataset_name', type=str, default='tcga_organ', choices=['TCGA', 'tcga_organ'], help='the dataset to be used.')
    ...
    parser.add_argument('--cmm_dim', type=int, default=512, help='the dimension of cmm.')
    ...
    # Mixed precision
    parser.add_argument('--amp', type=str, default='False', help='enable automatic mixed precision (True/False)')
...
def main(local_rank, world_size):
    ...
    # setup(local_rank, world_size, "nccl" if use_cuda else "gloo")
    if not args.debug:
    ...
    model = R2GenCMNModel(args, tokenizer).to(local_rank)
    ...
    if args.mode == 'Test':
        ...
        # checkpoint = torch.load(resume_path, map_location='cpu')['state_dict']
    ...
    # model = DDP(model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True) if use_cuda else DDP(model, find_unused_parameters=True)
